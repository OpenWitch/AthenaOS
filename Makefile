...
SRC_OS		:= os os/fs os/fs/rom os/il os/shell/stub
...
# BIOS defines
ifeq ($(BIOS_CATCH_IRQ_BOUNDS),true)
DEFINES		+= -DBIOS_CATCH_IRQ_BOUNDS
endif
ifeq ($(BIOS_CATCH_IRQ_ERRORS),true)
DEFINES		+= -DBIOS_CATCH_IRQ_ERRORS
endif
ifeq ($(BIOS_WORKAROUND_DIV_ZERO_VECTOR_CORRUPTION),true)
DEFINES		+= -DBIOS_WORKAROUND_DIV_ZERO_VECTOR_CORRUPTION
endif

DEFINES     += -DBIOS_BANK_MAPPER_$(BIOS_BANK_MAPPER)
DEFINES     += -DBIOS_BANK_ROM_FORCE_COUNT=$(BIOS_BANK_ROM_FORCE_COUNT)
ifeq ($(BIOS_BANK_ROM_FORCE_DYNAMIC),true)
DEFINES     += -DBIOS_BANK_ROM_FORCE_DYNAMIC
endif
ifeq ($(BIOS_BANK_QUIRK_CE_DISABLES_ROM),true)
...
ifeq ($(OS_ENABLE_128K_SRAM),true)
DEFINES     += -DOS_ENABLE_128K_SRAM
endif
